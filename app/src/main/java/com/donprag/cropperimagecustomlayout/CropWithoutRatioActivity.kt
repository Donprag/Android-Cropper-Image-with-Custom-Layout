...
import com.donprag.cropperimagecustomlayout.databinding.ActivityCropWithoutRatioBinding
import com.donprag.cropperimagecustomlayout.helper.loadImageFrom
import java.io.ByteArrayOutputStream

class CropWithoutRatioActivity : AppCompatActivity() {
    private lateinit var bind: ActivityCropWithoutRatioBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bind = ActivityCropWithoutRatioBinding.inflate(layoutInflater)
                ...
                MotionEvent.ACTION_MOVE -> {
                    if (isResizingTopLeft || isResizingTopRight || isResizingBottomLeft || isResizingBottomRight) {
                        ...
                        if (isResizingTopLeft) {
                            val newWidth = (originalWidth - (event.rawX - lastX)).toInt()
                            val newHeight = (originalHeight - (event.rawY - lastY)).toInt()

                            params.width = maxOf(newWidth, 100)
                            params.height = maxOf(newHeight, 100)

                            v.x = originalX + (event.rawX - lastX)
                            v.y = originalY + (event.rawY - lastY)
                        }
    ...
    private fun cropImage() {
        ...
        val intent = Intent(this@CropWithoutRatioActivity, ResultCropActivity::class.java)
